...
\end{satz}
\begin{proof}
Sei $D\subset \Omega$ eine abzählbare, dichte Menge und sei $(B(x_j, r_j))_{j\in \mathbb N}$ die Folge der abgeschlossenen Kugeln welche alle Kugeln $\overline{B(x, r)}$ mit $x \in D, r\in \mathbb Q$ und $\overline{B(x,r)}\subset U$ für ein $U\in \mathcal O$ umfasst.
...
\psi_2 &:= (1-\phi_1) \phi_2\\
&\vdots \\
\psi_i &:= (1-\phi_1)(1-\phi_2) \cdots (1- \phi_{i-1}) \phi_i.
\end{align*}

Dann gilt $0 \le \psi_j \le 1$, $\text{supp}(\psi_j) \subset \text{supp}(\phi_j) \subset \overline{B(x_j, r_j)}$ und
$$
\psi_1 + \psi_2 + \ldots + \psi_i = 1- \prod_{j=1}^i (1-\phi_j)
$$
\\
Da $\phi_i =1$ in $V_i$ folgt $\psi_1 + \psi_2 + \cdots + \psi_j =1$ in $V_1 \cup V_2 \cup V_3 \cup \ldots \cup V_j$. Sei $K\subset \Omega$ kompakt, dann existiert ein $m\in \mathbb N$ mit $K\subset \bigcup_{j=1}^m V_j =:W $, denn $\bigcup V_j = \Omega$.
\\ Also gilt
\begin{align*}
\sum_{i=1}^m \psi_i = 1 \ \ \text{in} \ \ W.
\end{align*}
\end{proof}
...
$$
\Omega_k = \{x\in \Omega |\ \text{dist} (x, \Omega^c) >\frac{1}{k} \ \  \text{ und } |x| <k\}.
$$
Dann gilt $\Omega_1 \subset \Omega_2 \subset \cdots \subset \Omega$ und $\bigcup_{k\ge 1} \Omega_k = \Omega$ für 
$k\ge 2$. Sei $$U_k: = \Omega_{k+1} \cap \overline{\Omega_{k-1}}^c = \Omega_{k+1} \setminus \overline{\Omega_{k-1}}$$ für 
$k\ge 2$
und $U_1= \Omega_1$. Dann $\Omega = \bigcup_{k=1}^\infty U_k$. Sei $(\phi_j)$ eine der offenen Überdeckung $\Omega = \bigcup_{i \ge 1} U_i$ untegeordnete lokal endliche Zerlegung der Eins (Satz 5) und sei $(\psi_k)_{k\ge 1}$ wie folgt definiert. $\psi_1$ ist die Summe der $\phi_i$ mit $\supp(\phi_i) \subset U_1$. $\phi_2$ ist die Summe der $\phi_i$ mit $\text{supp}(\phi_i) \subset U_2$ aber $\text{supp}(\phi_i) \not \subset U_1$ etc.  Dann ist $\psi_k \in C_0^\infty(\Omega)$ denn $\overline U_k$ kompakt und somit ist $\psi_k$ eine endliche Summe. Außerdem $0 \le \psi_k \le 1, \sum \psi_k(x)=1$ in $\Omega$, $\text{supp}(\psi_k) \subset U_k$.  Sei $\varepsilon>0$ und $\varepsilon_k >0$ so klein, dass  
$$
...
kompakt und
 ...
 Wir wollen nun Einbettungssätze für $W^{m,p}(\Omega)$ beweisen. Diese führen wir im nächsten Kapitel auf Einbettungssätze für $W^{1,p}(\Omega)$ zurück.  

\section{Fortsetzungsoperator}
...
\begin{equation}
E_{\eta, r} : C^\infty(\overline{B(0,\eta) \times (0,r)}) \cap W^{1,p}(B(0,R) \times (0,r)) \to W^{1,p}(B(0,R) \times (-r,r)).
\end{equation}
Dieser Operator ist beschränkt und lässt sich daher fortsetzen auf $W^{1,p}(B(0,R) \times (0,r))$ (Satz \ref{thm3.9}).

\textbf{Konstruktion von $E$}.  Da $\partial \Omega$ kompakt und Lipschitz ist, existiert eine offene Überdeckung $\partial \Omega \subset \bigcup_{i=1}^N U_i$ und Lipschitz-Abbildung $h: B(0,\eta_i) \to \mathbb R$, sodass (nach Verschiebung und Drehung) das Koordinaten-System 
\begin{equation}
\Omega \cap U_i = \{ (x', x_n) \in U_i | x_n > h_i(x'), x' \in B(0, \eta_i)\}.
\end{equation}
Wir dürfen annehmen, dass $U_I$ zylinderförmig ist, d.h. $U_i = B(0, \eta_i) \times (-\rho_i, \rho_i)$. Wir wählen $U_0 \subset \subset \Omega$, so dass 
\begin{equation}
\overline{\Omega} \subset \bigcup_{i=0}^N U_i
\end{equation}
und bezeichnen mit $(\phi_i)_{i=0}^N$ eine untergeoordnete Zerlegung der Eins, d.h. $\supp(\phi_i) \subset U_i, \sum_{i=1}^N \phi_i \equiv 1$ auf $\overline{\Omega}$.  Wir werden nachstehend beschränkte lineare Abbildung $E_i: W^{1,p}(\Omega \cap U_i) \to W_0^{1,p}(\mathbb R^n)$ konstruieren mit $E_i(\phi_i u)\big |_\Omega = \phi_i u$ für alle $u\in W^{1,p}(\Omega \cap U_i)$. 

Zur Konstruktion von $E$ wählen wir $\phi \in C_0^\infty(\Omega_1)$ mit $\phi =1$ auf $\overline{\Omega}$. Für $u\in W^{1,p}(\Omega)$ setzen wir
\begin{equation}
Eu = \Phi\left ( \sum_{i=1}^N E_i(\phi_i u) \right )\bigg |_{\Omega_1} + \phi_0 u
\end{equation}
wobei wir uns $\phi_0 u\in W_0^{1,p}(\Omega)$ durch Null auf $\Omega_1$ fortgesetzt denken. Dann ist $Eu\in W_0^{1,p}(\Omega_1)$,
\begin{equation}
Eu \big |_\Omega= \sum_{i=1} E_i (\phi_i u) \big |_\Omega + \phi_0 u= \sum_{i=1}^N(\phi_i u)+ \phi_0 u=u
\end{equation}
und $\| Eu\|_{W^{1,p}(\Omega_1)} \le C \| u\|_{W^{1,p}(\Omega)}$.

\textbf{Konstruktion von $E_i$}. Sei $\tilde U_i= \{ (x', x_n) | (x', x_n + h_i(x')) \in U_i\}$

Weil die Abbildung $(x', x_n) \mapsto (x', x_n \pm h_i (x'))$ Lipschitzstetig ist,, werden durch
\begin{align*}
T_i v(x', x_n) &= v(x', x_n + h(x'))\\
T_i' v(x', x_n) &= v(x', x_n - h(x'))
\end{align*}
beschränkte lineare Abbildung
\begin{equation}
T_i: W^{1,p}(\Omega \cap U_i) \to W^{1,p}(\mathbb R^n_+ \cap \tilde U_i)
\end{equation}
definiert (Satz \ref{thm4.5}) . Wir benötigen außerdem die beschränkte lineare Abbildung 
\begin{align}
S_i : W^{1,p}(\tilde U_i \cap \mathbb R^n_+) &\to W^{1,p}(\tilde U_i)\\
\psi_i: W^{1,p}(U_i) \to W_0^{1,p}(\mathbb R^n).
\end{align}
Dann ist
\begin{equation}
E_i = \psi_i \circ T_i' \circ S_i \circ T_i: W_0^{1,p}(\Omega) \cap U_i \to W_0^{1,p}(\mathbb R^n)
\end{equation}
beschränkt und nach Konstruktion (siehe unten)
\begin{equation}
T_i' S_i T_i \phi_i u\big |_\Omega = \phi_i u.
\end{equation}
Wir definieren $\psi_i$ durch Multiplikation mit einer Funktion $\psi_i' \in C_0^\infty(U_i)$ mit $\psi_i'=1$ auf $\supp(\phi_i)$ und anschließende Fortsetzung auf $\mathbb R$ durch Null. 

$S_i$ ist im Wesentlichen der Spiegelungsoperator. Genauer ist $S_i$ eine Abbildung von $W^{1,p}$-Räumen mit folgen Gebieten
\begin{equation}
\tilde U_i \cap \mathbb R_+^n \to B(0, \eta_i) \times (0,r) \to B(0, \eta_i) \times (-r,r) \to U_i.
\end{equation}
Im ersten Schritt multiplizieren wir mit einer Funktion $\tilde \phi_i\in C_0^\infty(\tilde U_i)$ und setzen dann durch Null fort auf $B(0, \eta_i) \times (0,r)$ wobei $\tilde \phi_i=1$ auf $\supp(T_i \phi_i u)$. Im zweiten Schritt wenden wir $E_{\eta_i, p_i}$ an. Im dritten Schritt multiplizieren wir wieder mit $\phi_i^2$ und schränkten die Funktion dann auf $\tilde U_i$ ein. Grob gesagt:
\begin{equation}
S_i= \tilde \phi E_{\eta, r} \tilde \phi_i.
\end{equation} 
Man überzeugt sich leicht, dass
\begin{equation}
T_i' S_i T_i \phi_i u = \phi_i u.
\end{equation}
\end{proof}

\section{Der Spuroperator}
Sei $\Omega \subset \mathbb R^n$ offen und beschränkt mit Lipschitzrand. Dann lässt sich $\partial \Omega$ ein Maß $\sigma$ definieren mit der Eigenschaft
\begin{equation}
\sigma(\Gamma) = \int_{g^{-1}(\Gamma)} \sqrt{1+|\nabla h(x)|^2}\, \mathrm dx
\end{equation}
wenn $\Gamma \subset \partial \Omega$ und $\partial \Omega \cap U$ der Graph einer Lipschitz-Abbildung $x\mapsto h(x)$ ist und $g(x) = (x, h(x))$ die zugehöriger Parametrisierung von $\partial \Omega \cap U$ ist (vgl. K. Jänich Vektoranalysis).

Somit ist auch $L^p(\partial \Omega) \to L^p(\partial\Omega, \sigma)$ erklärt und vollständig. Sei  $u\in C(\overline{\Omega}) \cap W^{1,p}(\Omega)$. Dann ist $u\big |_{\partial \Omega}$ eine stetige Funktion auf $\partial \Omega$ und die Abbildung
\begin{equation}
T: C(\overline{\Omega}) \cap W^{1,p}(\Omega) \to C(\partial \Omega), u\mapsto u\big |_{\partial \Omega}
\end{equation}
ist linear. Wir wollen diesen \glqq Spuroperator\grqq auf ganz $W^{1,p}(\Omega)$ fortsetzen.
...
\begin{proof}
Seien $T_1, T_2$ Spuroperatoren, dann gilt $T_1 u=T_2u$ für alle $U\in C(\overline{\Omega}) \cap W^{1,p}(\Omega)$. Da $C(\overline{\Omega}) \cap W^{1,p}(\Omega) \supset C^\infty(\overline{\Omega}) \cap W^{1,p}(\Omega)$ dicht in $W^{1,p}(\Omega)$ ist (Satz \ref{thm3.9}), folgt aus der Beschränktheit von $T_1, T_2$, dass $T_1 = T_2$ auf $W^{1,p}(\Omega)$. Sei $\partial \Omega = \bigcup_{i=1}^N U_i$ eine offene Überdeckung von $\partial \Omega$, so dass $\partial \Omega \cap U_i$ Graph einer Lipschitz-Abbildung $h_i: B(0, \eta_i) \to \mathbb R$ ist (bis auf Drehung/ Verschiebung).  Sei $\phi_1, \ldots, \phi_N$ eine untergeordnete Zerlegung der Eins. Wir definieren auf $C(\overline{\Omega}) \cap W^{1,p}(\Omega)$ durch $Tu = u\big |_{\partial \Omega_1}$. 

Es ist $\| Tu\|_{L^p(\partial \Omega} \le C \| u\|_{1,p}$ zu zeigen.  Sei $u\in C(\overline{\Omega}) \cap W^{1,p}(\Omega)$ gilt
\begin{equation}
\| Tu\|_p = \| T \sum_{i=1}^N \phi_i u\|_p \le \sum_{i=1}^N \| T\underbrace{\phi_i u}_{=u_i}\|_p,
\end{equation} 
wobei 
\begin{equation}
\| Tu_i\|_p^p = \int_{B(0,R_i)} |u_i(x', h(x'))|^p \sqrt{1+ (\nabla h_i)^2} \, \mathrm dx' \le C_i \int B(0, \eta_i) |u_i(x', h(x'))|^p \, \mathrm dx'
\end{equation}
nach Satz \ref{thm4.4} und 
\begin{align*}
|u_i(x', h(x'))|^p &= \int_{h(x')}^r \frac{\partial}{\partial x_n} |u_i(x', x_n)|^p\, \mathrm dx_n \\
&= \int_{h(x')}^r \frac{p}{2} |u_i(x', x_n)|^{p-2} ((\partial_n \overline {u_i}) u_i + \overline{u_i} \partial_n u_i)\\
&\le \int_{h(x')}^r (p-1) (|u_i|^p + |\nabla u_i|^p) \, \mathrm dx_n
\end{align*}
für ein $r>0$ geeignet gewählt, sodass $u_i(x', r)=0$ für alle $x' \in B(0, \eta)$. Zuletzt haben wir die Youngsche Ungleichung in der Form
\begin{equation}
a^{p-1} b \le \frac{p-1}{p} a^p + \frac{1}{p} b^p
\end{equation}
verwendet.  Also gilt
\begin{align*}
\int_{B(0, \eta_i)} |u_i(x', h_i(x'))|^p \, \mathrm dx' \le C \int_{B(0, \eta_i)} \int_{h_i(x')}^r ( |u_i|^p + |\nabla u_i|^p ) \, \mathrm dx_n \, \mathrm dx' \le C \| u_i\|_{W^{1,p}(\Omega\cap U_i)}^p.
\end{align*}

Wir schließen daraus, dass $\| Tu\| \le C\|u\|_{1,p}$. $T$ ist also beschränkt und wegen der Vollständigkeit von $L^p(\partial \Omega)$ auf ganz $W^{1,p}(\Omega)$ fortsetzbar.
\end{proof}
...
&\le \| \psi\| \cdot \| \chi_{B_R} \eta_n\| + C \varepsilon
\end{align*}

...
\\ Es gilt
\begin{align*}
\int \left(\partial^\alpha \overline{\phi}\right) u \mathrm{d}x &= <\partial^\alpha \phi, u> = <\widehat{\partial^\alpha \phi}, \widehat{u}> = <(i ~\cdot~)^\alpha \widehat{\phi}, \widehat{u} > = \int \overline{(ik)^\alpha \widehat{\phi}}(k) \cdot\widehat{u}(k)\mathrm{d}k \\& = (-1)^{|\alpha|} \int \widehat{\phi}(k) \widehat{u}(k) (ik)^\alpha \mathrm{d}k = (-1)^{|\alpha|} < \widehat{\phi},(i ~\cdot~)^\alpha \widehat{u} > 
\\& = (-1)^{|\alpha|} <\phi, u_\alpha>.
\end{align*}
\ 
\\$(b) $ \ Siehe Aufgabe $6.1$
\end{proof}
...
<u,v>_s := \int \overline{\widehat{u}(k)}\widehat{v}(k)(1+|k|^2)^s \mathrm{d}k 
\end{equation}
definiert.
\end{df}
\begin{thm}
